...
class CloseBridgeController extends GetxController {
  ...
  Future<void> loadModel() async {
    try {
      ...
      debugPrint('✅ Model CloseBridge loaded');
    } catch (e) {
  ...
  Future<void> initializeCamera(int cameraIndex) async {
    try {
      ...
      cameraController = CameraController(
        selectedCamera,
        ResolutionPreset.low, // Gunakan resolusi rendah untuk performa
        enableAudio: false,
  ...
  void processCameraImage(CameraImage image) async {
    ...
    try {
      ...
      predictedLabel.value = score > 0.85 ? 'CloseBridge' : 'unknown';
    } catch (e) {
      debugPrint("❌ Detection error: $e");
    } finally {
      // Beri delay ringan untuk menghindari frame terlalu rapat
      await Future.delayed(const Duration(milliseconds: 150));
      isDetecting = false;
    }
  }
